...
$subjects_query = "SELECT DISTINCT subject 
                   ...
                   WHERE approval_status = 'approved' AND visibility_status = 'yes'";
...
$sql = "SELECT teachers.*, countries.name AS country_name 
        FROM teachers 
        JOIN countries ON teachers.country_id = countries.id
        WHERE approval_status = 'approved' 
        AND visibility_status = 'yes'";
...
if ($video) {
    $sql .= " AND video = '" . $conn->real_escape_string($video) . "'";
}
...
<head>  
    ...
    <style>  
        ...
        /* Header Section */  
        header {  
            background-color: #6C3D66; /* Dark Purple background for header */  
            color: white;  
            padding-left: 10px;
            padding-right: 10px;  
            text-align: center;  
        ...
        .logo {  
            font-size: 36px; /* Adjusted size for logo */  
            font-weight: bold;  
            color: white;  
        }  
        ...
        /* Button styles */  
        .btn-log-in {  
            background-color: #9B59B6 !important; /* Purple button */  
            border-color: #9B59B6 !important; /* Purple border */  
        ...
        /* Links */  
        a {  
            color: #4d3346; /* Soft Purple for links */  
            text-decoration: none;  
        }  

        a:hover {  
            color: #9B59B6; /* Accent purple when hovered */  
        }  
        ...
        .dropdown-menu {  
            background-color: #D8A7CA; /* Light Lilac background for dropdown */  
        }  

        .dropdown-item {  
            color: #4d3346; /* Soft Purple for dropdown items */  
        }  

        .dropdown-item:hover {  
            background-color: #C084B1; /* Hover effect for dropdown items */  
            color: white;  
        }  
...
<header class="d-flex justify-content-between align-items-center">  
    <div class="logo">Preply</div>  
    <nav class="d-flex align-items-center">  
        <a class="nav-link" href="find_tutors.php">Find tutors</a>  
         
        <a class="nav-link" href="..\teacher\Sign_up.php">Become a tutor</a>  
        <a class="nav-link" href="..\teacher\Sign_up.php">Sign up</a>  
        
        

        
        
        <a class="btn btn-log-in" href="..\teacher\login.php">Log In</a>  
    </nav>  
</header>  
...
if ($result->num_rows > 0) {  
    while ($row = $result->fetch_assoc()) {  
        ...
        echo '<a href="trial_lesson_page.php" class=" d-flex tutor-card " 
                ...
                onclick="storeTeacherId(' . htmlspecialchars($row['id']) . ')">'; 
        ...
        echo '<div><strong>Speaks:</strong> ' . htmlspecialchars($row['languages']) . '</div>';
        echo '</div>';
        ...
        echo '<div class="tutor-price">US$' . htmlspecialchars($row['pricing']) . '/hr</div>';
        ...
        echo '<input type="hidden" name="teacher_id" value="' . htmlspecialchars($row['id']) . '">';
        ...
        echo '</div>'; // End of the tutor card
                ...
                echo '<iframe src=" '. htmlspecialchars($row['video']) . '" allowfullscreen></iframe>';
                echo '</div>';
                echo '</div>';  
